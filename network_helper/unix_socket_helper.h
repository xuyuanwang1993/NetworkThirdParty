...
public:
     ...
     * @return if fd hasn't been set up,it will return INVALID_SOCKET(0xffffffff)
     */
    SOCKET fd()const{return m_fd;}
    /**
     * @brief build set up the socket
     * @return it will return INVALID_SOCKET(0xffffffff) when fail setting up the socket
     */
