...
file_reader_base::file_reader_base(string fileName)
{
    ...
    if(m_fp)parase_stream_info();
        ...
        MICAGENT_LOG1(PRINT_RED "Can't open %s!",fileName.c_str());
    }
}
...
//    gettimeofday(&time_now,nullptr);
    ...
    do{
        ...
        {
            MICAGENT_LOG1(PRINT_RED "Read frame %lu error!",index);
            break;
        }
        m_last_index+=1;
        ...
        {
            MICAGENT_LOG1(PRINT_RED "inBufSize is too small! It needs %lu at least but now %u is available!",frame_size,inBufSize);
            break;
        }
        ...
        {
            ret=0;
            MICAGENT_LOG1(PRINT_RED " IO errors! %s ",strerror(errno));
            break;
        }
...
//        MICAGENT_LOG1(PRINT_GREEN "readFrame using time %lld microseconds!",us);
    }while(0);
    return  ret;
}
void file_reader_base::show_a_frame_info(size_t index,size_t max_out_put_size)const
{
    ...
    if(ret==0)MICAGENT_LOG1(PRINT_RED "Get %lu frame info error!",index);
}
...
void file_reader_base::dump_file_info(size_t items)const
{
    ...
    MICAGENT_LOG1(PRINT_GREEN "now is all %lu frames info!",limit);
    if(items!=0)limit=limit<items?limit:items;
    for(size_t i=0;i<limit;i++)
    {
        MICAGENT_LOG1(PRINT_YELLOW "frame:%lu    begin_pos: %lu",i,m_frame_begin_poses[i]);
    }
    MICAGENT_LOG1(PRINT_GREEN "now is all %lu types info!",m_frame_info_map.size());
    for(auto i : m_frame_info_map)
    {
        limit=i.second.size();
        MICAGENT_LOG1(PRINT_LIGHT_RED "frame_type : %02x counts : %lu",i.first,limit);
        if(items!=0)limit=limit<items?limit:items;
        for(size_t j=0;j<limit;j++)
        {
            MICAGENT_LOG1(PRINT_YELLOW "frame:%lu    begin_pos: %lu",j,i.second[j]);
        }
    }
}
...
void file_reader_base::parase_stream_info()
{
    ...
    do{
        if(!m_fp)
        {
            MICAGENT_LOG1(PRINT_RED "%s is not opened!",m_file_name.c_str());
            break;
        }
        ...
        if(read_size<=0)
        {
            MICAGENT_LOG1(PRINT_RED "%s is empty!",m_file_name.c_str());
            break;
        }
