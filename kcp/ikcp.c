...
const IUINT32 IKCP_WND_SND = 8192;//1024
const IUINT32 IKCP_WND_RCV = 8192;       //1024  must >= max fragment size
const IUINT32 IKCP_MTU_DEF = 1400;
const IUINT32 IKCP_ACK_FAST	= 3;
const IUINT32 IKCP_INTERVAL	= 100;
const IUINT32 IKCP_OVERHEAD = 24;       //kcp协议头字节长度
...
static inline const char *ikcp_decode32u(const void *p, IUINT32 *l)
{
#if IWORDS_BIG_ENDIAN
...
{//评估网络延时
    ...
    if (kcp->rx_srtt == 0) {
        ...
        kcp->rx_rttval = (3 * kcp->rx_rttval + delta) / 4;//计算网络抖动
        kcp->rx_srtt = (7 * kcp->rx_srtt + rtt) / 8;//计算平均延时
