...
using MEDIA_SESSION_CALLBACK=function<void(string url_name)>;
};
using std::endl;
using std::cout;
...
//此函数的返回值为取消会话和数据传输所需传入的sessionid
 ...
 typedef  enum
    ...
    //设置媒体源发生改变时的回调函数
    static void Api_Rtsp_Set_MediaSession_Callback(std::shared_ptr<Api_rtsp_server::Rtsp_Handle> handle,const micagent::MEDIA_SESSION_CALLBACK &new_session_cb,\
